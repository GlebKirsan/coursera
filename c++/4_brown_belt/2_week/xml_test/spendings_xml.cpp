...
vector<Spending> LoadFromXml(istream& input) {
    ...
    for (auto& children : doc.GetRoot().Children()) {
        string category{children.AttributeValue<string>("category")};
        auto value = children.AttributeValue<int>("amount");
        spendings.push_back({move(category), value});
    }
    return spendings;
}
