...
public:
    ...
    HttpResponse& AddHeader(string name, string value) {
        response.headers.push_back({move(name), move(value)});
        return *this;
    }
    HttpResponse& SetContent(string a_content) {
        response.content = move(a_content);
        return *this;
    }
...
ostream& operator<<(ostream& output, const HttpResponse& resp) {
    output << "HTTP/1.1 ";
    output << to_string(resp.response.code) << " ";
    switch (resp.code) {
        case HttpCode::Ok:
            output << "OK\n";
            break;
    ...
    for (const auto&[name, value] : resp.response.headers) {
        output << name << ": " << value << "\n";
    }
    if (!resp.response.content.empty()) {
        output << "Content-Length: " << resp.response.content.size() << "\n";
    ...
    HttpResponse ServeRequest(const HttpRequest& req) {
        ...
        if (req.method == "POST") {
            if (req.path == "/add_user") {
                ...
                response.SetCode(HttpCode::Ok).SetContent(id);
            } else if (req.path == "/add_comment") {
                auto[user_id, comment] = ParseIdAndContent(req.body);
                if (!last_comment || last_comment->user_id != user_id) {
                    last_comment = LastCommentInfo{user_id, 1};
            ...
            if (req.path == "/captcha") {
                response.SetCode(HttpCode::Ok)
                        .SetContent(
                                "What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
            } else if (req.path == "/user_comments") {
                auto user_id = FromString<size_t>(req.get_params.at("user_id"));
                string comments;
                for (const string& c : comments_[user_id]) {
                    comments += c + '\n';
                }
                response.SetCode(HttpCode::Ok).SetContent(comments);
            }
        }
        return response;
    }
