...
struct AddressHasher {
    size_t operator()(const Address& address) const {
        return string_hasher(address.street) * x * x +
               string_hasher(address.city) * x +
...
struct PersonHasher {
    size_t operator()(const Person& person) const {
        return string_hasher(person.name) * x * x * x +
               double_hasher(person.weight) * x * x +
               int_hasher(person.height) * x +
